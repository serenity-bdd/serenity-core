 ...
 * Time: 9:57 AM
 */
class WhenExecutingPutRequestsFromSerenityRest extends Specification {
    ...
    def "should return wrapped response during PUT by URL called from Serenity Rest"() {
    ...
    def "should return wrapped response during PUT by URI called from Serenity Rest"() {
        given: "configured access point"
            def body = "<root>" +
                "<value>2</value>" +
                "</root>"
    ...
    def "should return wrapped response during PUT by base URL called from Serenity Rest"() {
        given: "configured access point"
            ...
            def path = "/test/get/keyboard"
            def url = "$base$path"
            stubFor(WireMock.put(urlMatching(".*"))
                .withRequestBody(matching(".*"))
                .willReturn(aResponse()
                .withStatus(256)
                .withHeader("Content-Type", "application/xml")
    ...
    def "should return wrapped response during PUT by path and parameters called from Serenity Rest"() {
        given: "configured access point"
