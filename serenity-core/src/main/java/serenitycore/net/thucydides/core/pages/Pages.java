...
import serenitycore.net.serenitybdd.core.pages.PageObjects;
import serenitymodel.net.thucydides.core.annotations.Fields;
import serenitycore.net.thucydides.core.steps.EnclosingClass;
...
import serenitycore.net.thucydides.core.webdriver.WebdriverProxyFactory;
...
public class Pages implements Serializable {
    ...
    serenitycore.net.serenitybdd.core.pages.PageObject currentPage = null;

    public <T extends serenitycore.net.serenitybdd.core.pages.PageObject> T getAt(final Class<T> pageObjectClass) {
        return getPage(pageObjectClass);
    }

    public <T extends serenitycore.net.serenitybdd.core.pages.PageObject> T getPage(final Class<T> pageObjectClass) {
	...
	public <T extends serenitycore.net.serenitybdd.core.pages.PageObject> T get(final Class<T> pageObjectClass) {
        T nextPage;
    ...
    @SuppressWarnings("unchecked")
    public <T extends serenitycore.net.serenitybdd.core.pages.PageObject> T currentPageAt(final Class<T> pageObjectClass) {
        T nextPage;
        if (shouldUsePreviousPage(pageObjectClass)) {
            nextPage = (T) currentPage;
        } else {
    ...
    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> void  openBrowserIfRequiredFor(T pageCandidate) {
        if (browserNotOpen()) {
    ...
    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> void checkUrlPatterns(Class<T> pageObjectClass, T pageCandidate) {
    ...
    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> boolean shouldUsePreviousPage(final Class<T> pageObjectClass) {
        if (!usePreviousPage) {
            return false;
        } else {
    ...
    private void cacheCurrentPage(serenitycore.net.serenitybdd.core.pages.PageObject newPage) {
        this.currentPage = newPage;
    }

    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> boolean currentPageIsSameTypeAs(Class<T> pageObjectClass) {
        return (currentPage != null) && (currentPage.getClass().equals(pageObjectClass));
    }

    public boolean isCurrentPageAt(final Class<? extends serenitycore.net.serenitybdd.core.pages.PageObject> pageObjectClass) {
        try {
            serenitycore.net.serenitybdd.core.pages.PageObject pageCandidate = getCurrentPageOfType(pageObjectClass);
            String currentUrl = getDriver().getCurrentUrl();
     ...
     */
    @SuppressWarnings("unchecked")
    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> T getCurrentPageOfType(final Class<T> pageObjectClass) {
        T currentPage = null;
        try {
    ...
    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> T createFromSimpleConstructor(Class<T> pageObjectClass)
            throws IllegalAccessException, InvocationTargetException, InstantiationException {
        T newPage = null;
        try {
            if (hasDefaultConstructor(pageObjectClass)) {
                Class[] constructorArgs = new Class[0];
                Constructor<? extends serenitycore.net.serenitybdd.core.pages.PageObject> constructor = pageObjectClass.getDeclaredConstructor(constructorArgs);
                constructor.setAccessible(true);
                newPage = (T) constructor.newInstance();
                newPage.setDriver(getDriver());
            } else if (hasOuterClassConstructor(pageObjectClass)) {
                Constructor<? extends serenitycore.net.serenitybdd.core.pages.PageObject> constructor = pageObjectClass.getDeclaredConstructor(new Class[] {pageObjectClass.getEnclosingClass()});
                constructor.setAccessible(true);
    ...
    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> boolean hasDefaultConstructor(Class<T> pageObjectClass) {
        return Arrays.stream(pageObjectClass.getDeclaredConstructors())
                .anyMatch( constructor -> constructor.getParameters().length == 0 );
    }


    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> boolean hasOuterClassConstructor(Class<T> pageObjectClass) {
    ...
    private <T extends serenitycore.net.serenitybdd.core.pages.PageObject> T createFromConstructorWithWebdriver(Class<T> pageObjectClass)
            throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        Class[] constructorArgs = new Class[1];
        constructorArgs[0] = WebDriver.class;
        Constructor<? extends serenitycore.net.serenitybdd.core.pages.PageObject> constructor = pageObjectClass.getConstructor(constructorArgs);
    ...
    private void thisPageObjectLooksDodgy(final Class<? extends serenitycore.net.serenitybdd.core.pages.PageObject> pageObjectClass, String message) {

        String errorDetails = "The page object " + pageObjectClass + " looks dodgy:\n" + message;
        throw new WrongPageError(errorDetails);
    }

    private void thisIsNotThePageYourLookingFor(final Class<? extends serenitycore.net.serenitybdd.core.pages.PageObject> pageObjectClass) {
