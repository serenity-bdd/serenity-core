package serenitymodel.net.thucydides.core.logging;
...
public class ConsoleLoggingListener implements StepListener {
    ...
    public void testStarted(String description) {
        ...
        nestedSteps.clear();
        if (loggingLevelIsAtLeast(LoggingLevel.NORMAL)) {
            getLogger().info(consoleHeading.bannerFor(ConsoleEvent.TEST_STARTED, description));
        }
    }
    ...
    public void testStarted(String description, String id) {
        if (loggingLevelIsAtLeast(LoggingLevel.NORMAL)) {
            getLogger().info(consoleHeading.bannerFor(
                ConsoleEvent.TEST_STARTED, description + "(" + id + ")"));
        }
    }
    ...
    private void logFailure(TestOutcome result) {
        if (loggingLevelIsAtLeast(LoggingLevel.QUIET)) {
            getLogger().error(red(consoleHeading.bannerFor(ConsoleEvent.TEST_FAILED, result.getTitle())));
            logRelatedIssues(result);
    ...
    private void logError(TestOutcome result) {
        if (loggingLevelIsAtLeast(LoggingLevel.QUIET)) {
            getLogger().error(red(consoleHeading.bannerFor(ConsoleEvent.TEST_ERROR, result.getTitle())));
            logRelatedIssues(result);
            logFailureCause(result);
    ...
    private void logCompromised(TestOutcome result) {
        if (loggingLevelIsAtLeast(LoggingLevel.QUIET)) {
            getLogger().error(red(consoleHeading.bannerFor(ConsoleEvent.TEST_COMPROMISED, result.getTitle())));
            logRelatedIssues(result);
            logFailureCause(result);
        }
    }
    ...
    private void logPending(TestOutcome result) {
        if (loggingLevelIsAtLeast(LoggingLevel.SUMMARY)) {
            getLogger().info(cyan(consoleHeading.bannerFor(ConsoleEvent.TEST_PENDING, result.getTitle())));
        }
    }

    private void logSkipped(TestOutcome result) {
        if (loggingLevelIsAtLeast(LoggingLevel.SUMMARY)) {
            getLogger().info(yellow(consoleHeading.bannerFor(ConsoleEvent.TEST_SKIPPED, result.getTitle())));
        }
    }

    private void logSuccess(TestOutcome result) {
        if (loggingLevelIsAtLeast(LoggingLevel.SUMMARY)) {
            getLogger().info(green(consoleHeading.bannerFor(ConsoleEvent.TEST_PASSED, result.getTitle())));
        }
    }
