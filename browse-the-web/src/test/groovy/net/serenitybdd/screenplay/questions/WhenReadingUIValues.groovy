...
class WhenReadingUIValues extends Specification {
    ...
    def "should read string values"() {
        ...
        then:
            Value.of(target).onTheScreenOf(actor).as(String) == "some value"
    }
    ...
    def "should read boolean values"() {
        ...
        then:
            Value.of(target).onTheScreenOf(actor).asBoolean() == true
        and:
            Value.of(target).onTheScreenOf(actor).as(Boolean) == true
    }
    ...
    def "should convert string values to numerical types"() {
        when:
            element.getValue() >> "1"
        then:
            Value.of(target).onTheScreenOf(actor).as(Integer) == 1
            Value.of(target).onTheScreenOf(actor).as(Long) == 1L
            Value.of(target).onTheScreenOf(actor).as(Double) == 1.0D
            Value.of(target).onTheScreenOf(actor).as(Float) == 1.0F
    }
    ...
    def "should convert string values to numerical types using shortcut methods"() {
        ...
        element.getValue() >> "1"
        then:
        Value.of(target).onTheScreenOf(actor).asInteger() == 1
        Value.of(target).onTheScreenOf(actor).asDouble() == 1.0D
    }
    ...
    def "should convert string values to dates"() {
        ...
        then:
            Value.of(target).onTheScreenOf(actor).asDate() == new DateTime(2015,10,15,0,0)
    }
    ...
    def "should convert string values to formatted dates"() {
        ...
        then:
           Value.of(target).onTheScreenOf(actor).asDate("dd/MM/yyyy") == new DateTime(2015,10,15,0,0)
    }
    ...
    def "should convert string values to other enums"() {
        ...
        then:
            Value.of(target).onTheScreenOf(actor).asEnum(Color.class) == Color.red
    }
    ...
    def "should throw assertion error if the enum is not an expected value"() {
        ...
        when:
            Value.of(target).onTheScreenOf(actor).asEnum(Color.class)
        then:
    ...
    def "should convert lists of string values"() {
        when:
            ...
            element2.getValue() >> "B"
        then:
            Value.of(target).onTheScreenOf(actor).asList() == ["A", "B"]
    }
    ...
    def "should convert lists of string enums"() {
        ...
        then:
            Value.of(target).onTheScreenOf(actor).asListOf(Color) == [Color.red, Color.blue]
    }
    ...
    def "should read text values"() {
        ...
        then:
            Text.of(target).onTheScreenOf(actor).as(String) == "some value"
    }
    ...
    def "should read multiple text values"() {
        ...
        then:
            Text.of(target).onTheScreenOf(actor).asList() == ["some value", "some other value"]
    }
    ...
    def "should read native text values"() {
        when:
            element.getText() >> "some value"
        then:
            Text.of(target).onTheScreenOf(actor).value() == "some value"
    }
    ...
    def "should read selected values"() {
        ...
        then:
            SelectedValue.of(target).onTheScreenOf(actor).as(String) == "some value"
    }
    ...
    def "should read multiple selected values"() {
        ...
        then:
            SelectedValue.of(target).onTheScreenOf(actor).asList() == ["some value", "some other value"]
    }
    ...
    def "should read visible selected values"() {
        ...
        then:
        SelectedVisibleTextValue.of(target).onTheScreenOf(actor).as(String) == "some value"
    }
    ...
    def "should read multiple svisible elected values"() {
        ...
        then:
        SelectedVisibleTextValue.of(target).onTheScreenOf(actor).asList() == ["some value", "some other value"]
    }
    ...
    def "should read selected options as string"() {
        ...
        then:
            SelectedOptions.of(target).onTheScreenOf(actor).as(String) == "[value1, value2]"
    }
    ...
    def "should read selected options"() {
        when:
            element.getSelectOptions() >> ["value1", "value2"]
        then:
            SelectedOptions.of(target).onTheScreenOf(actor).value() == ["value1", "value2"]
    }
    ...
    def "should read multiple selected options"() {
        ...
        then:
            SelectedOptions.of(target).onTheScreenOf(actor).asList() == [["value1", "value2"], ["value3", "value4"]]
    }
    ...
    def "should read css value"() {
        ...
        then:
            CSSValue.of(target).named("font").onTheScreenOf(actor).value() == "Italics"
    }
    ...
    def "should read multiple css values"() {
        ...
        then:
            CSSValue.of(target).named("font").onTheScreenOf(actor).asList() == ["Italics","Bold"]
    }
    ...
    def "should read attribute value"() {
        ...
        then:
            Attribute.of(target).named("checked").onTheScreenOf(actor).as(Boolean) == true
    }
    ...
    def "should read multiple attribute values"() {
        ...
        then:
            Attribute.of(target).named("checked").onTheScreenOf(actor).asListOf(Boolean.class) == [true,false]
    }
    ...
    def "should read size value"() {
        ...
        then:
            TheSize.of(target).onTheScreenOf(actor).value() == new Dimension(10,10)
    }
    ...
    def "should read multiple size values"() {
        ...
        then:
            TheSize.of(target).onTheScreenOf(actor).asList() == [new Dimension(10,10), new Dimension(20,20)]
    }
    ...
    def "should read location value"() {
        ...
        then:
            TheLocation.of(target).onTheScreenOf(actor).value() == new Point(10,10)
    }
    ...
    def "should read multiple location values"() {
        ...
        then:
            TheLocation.of(target).onTheScreenOf(actor).asList() == [new Point(10,10), new Point(20,20)]
    }
    ...
    def "should read coordinated value"() {
        ...
        then:
            TheCoordinates.of(target).onTheScreenOf(actor).value() == coordinates1
    }
    ...
    def "should read multiple coordinated values"() {
        ...
        then:
            TheCoordinates.of(target).onTheScreenOf(actor).asList() == [coordinates1, coordinates2]
    }
    ...
    def "should see if enabled"() {
        ...
        then:
            Enabled.of(target).onTheScreenOf(actor).value()== true
    }
    ...
    def "should see if selected"() {
        ...
        then:
            SelectedStatus.of(target).onTheScreenOf(actor).value()== true
    }
    ...
    def "should see if multiple fields are selected"() {
        ...
        then:
            SelectedStatus.of(target).onTheScreenOf(actor).asList()== [true,false]
    }
    ...
    def "should see if multiple elements enabled"() {
        ...
        then:
            Enabled.of(target).onTheScreenOf(actor).asList() == [true, false]
    }
    ...
    def "should see if currently enabled"() {
        ...
        then:
            CurrentlyEnabled.of(target).onTheScreenOf(actor).value()== true
    }
    ...
    def "should see if multiple elements currently enabled"() {
        ...
        then:
        CurrentlyEnabled.of(target).onTheScreenOf(actor).asList() == [true, false]
    }
    ...
    def "should see if currently visible"() {
        ...
        then:
        CurrentVisibility.of(target).onTheScreenOf(actor).value()== true
    }
    ...
    def "should see if multiple elements currently visible"() {
        ...
        then:
        CurrentVisibility.of(target).onTheScreenOf(actor).asList() == [true, false]
    }
    ...
    def "should see if visible"() {
        ...
        then:
            Visibility.of(target).onTheScreenOf(actor).value()== true
    }
    ...
    def "should see if multiple elements visible"() {
        ...
        then:
            Visibility.of(target).onTheScreenOf(actor).asList() == [true, false]
    }
    ...
    def "should see if present"() {
        ...
        then:
        Presence.of(target).onTheScreenOf(actor).value()== true
    }
    ...
    def "should see if multiple elements present"() {
        ...
        then:
        Presence.of(target).onTheScreenOf(actor).asList() == [true, false]
    }
