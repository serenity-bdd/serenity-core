 ...
 * Time: 9:57 AM
 */
class WhenExecutingDeleteRequestsFromSerenityRest extends Specification {
    ...
    def "should return wrapped response during DELETE by URL called from Serenity Rest"() {
        ...
        when: "creating new request and making get request"
            def response = delete(new URL(url))
    ...
    def "should return wrapped response during DELETE by URI called from Serenity Rest"() {
        given: "configured access point"
            def body = "<root>" +
                "<value>2</value>" +
                "</root>"
            ...
            stubFor(WireMock.delete(urlMatching(path))
                ...
                .withBody(body)))
        when: "creating new request and making get request"
            def response = delete(new URI(url))
        then: "created response should be decorated"
            response instanceof ResponseDecorated
        and: "returned status should be correct"
            response.then().statusCode(256)
    ...
    def "should return wrapped response during DELETE by base URL called from Serenity Rest"() {
        given: "configured access point"
            ...
            def path = "/test/get/keyboard"
            def url = "$base$path"
            stubFor(WireMock.delete(urlMatching(".*"))
                .withRequestBody(matching(".*"))
                .willReturn(aResponse()
                .withStatus(256)
                .withHeader("Content-Type", "application/xml")
    ...
    def "should return wrapped response during DELETE by path and parameters called from Serenity Rest"() {
        given: "configured access point"
            ...
            stubFor(WireMock.delete(urlMatching("$path.*"))
                ...
                .withBody(body)))
        when: "creating new request and making get request"
            def response = delete("$url?status={status}", ["status": "available"])
        then: "created response should be decorated"
            response instanceof ResponseDecorated
        and: "returned status should be correct"
            response.then().statusCode(256)
