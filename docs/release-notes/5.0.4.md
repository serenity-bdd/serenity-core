...
**JUnit 5 enhancements, Cucumber Scenario Outline fixes, report improvements, and Kotlin API enhancements**

This release adds support for JUnit 5 `@TestFactory` dynamic tests, fixes conditionally disabled tests missing from reports, resolves console logging indentation issues in parameterized tests, fixes Scenario Outline examples incorrectly shown as skipped, corrects DataTables errors in result-filtered reports, and improves the Kotlin developer experience for the `serenity-ensure` module.
...
### Kotlin-Friendly Ensure API
- Added `Ensure` object for Kotlin developers to use the more discoverable `Ensure.that(...)` syntax
- Previously, Kotlin users had to import top-level functions directly: `import net.serenitybdd.screenplay.ensure.that`
- Now Kotlin developers can use the idiomatic pattern:
  ```kotlin
  ...
  ```
- Java API remains unchanged - Java developers continue using static imports
- Fixes #3682

### Option to Hide Example Tables in Reports
- Added `serenity.report.show.example.tables` property to control visibility of Example tables in Scenario Outline reports
- Set to `false` in `serenity.properties` to hide the tables: `serenity.report.show.example.tables=false`
- Default is `true` (tables are shown)
- Useful for users with large Example tables (50+ rows) where the tables create visual clutter
- Fixes #3677
...
- **Better Kotlin developer experience**: Kotlin developers can now use the familiar `Ensure.that(...)` pattern for better IDE discoverability and consistency with Java usage
- **Cleaner reports for large data sets**: Users with many Scenario Outline examples can now hide the example tables to reduce visual clutter
