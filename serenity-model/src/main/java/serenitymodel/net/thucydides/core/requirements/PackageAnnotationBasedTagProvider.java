...
import serenitymodel.net.thucydides.core.requirements.annotations.NarrativeFinder;
...
import static serenitymodel.net.thucydides.core.ThucydidesSystemProperty.THUCYDIDES_TEST_ROOT;
import static serenitymodel.net.thucydides.core.reflection.ClassFinder.loadClasses;
...
public class PackageAnnotationBasedTagProvider extends AbstractRequirementsTagProvider implements RequirementsTagProvider, OverridableTagProvider {
    ...
    private void addRequirementTo(Map<String, Requirement> requirementsByPath,
        ...
        for (int level = 0; level < packageNames.length; level++) {
            currentPath = (currentPath.isEmpty()) ? packageNames[level] : Joiner
                .on(".").join(currentPath, packageNames[level]);
