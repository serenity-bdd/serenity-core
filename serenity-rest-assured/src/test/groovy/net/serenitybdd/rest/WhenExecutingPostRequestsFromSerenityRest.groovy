 ...
 * Time: 9:57 AM
 */
class WhenExecutingPostRequestsFromSerenityRest extends Specification {
    ...
    def "should return wrapped response during POST by URL called from Serenity Rest"() {
    ...
    def "should return wrapped response during POST by URI called from Serenity Rest"() {
        given: "configured access point"
            def body = "<root>" +
                "<value>2</value>" +
                "</root>"
    ...
    def "should return wrapped response during POST by base URL called from Serenity Rest"() {
        given: "configured access point"
    ...
    def "should return wrapped response during POST by path and parameters called from Serenity Rest"() {
        given: "configured access point"
            ...
            stubFor(WireMock.post(urlMatching("$path.*"))
                ...
                .withBody(body)))
        when: "creating new request and making get request"
            def response = post("$url?status={status}", ["status": "available"])
        then: "created response should be decorated"
            response instanceof ResponseDecorated
        and: "returned status should be correct"
            response.then().statusCode(256)
