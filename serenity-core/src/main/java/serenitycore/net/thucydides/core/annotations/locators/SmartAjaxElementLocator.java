...
import serenitycore.net.thucydides.core.webdriver.exceptions.ElementNotVisibleAfterTimeoutError;
...
public class SmartAjaxElementLocator extends SmartElementLocator implements WithConfigurableTimeout {
    ...
    private final static SearchContextProvider WEB_DRIVER_FACADE = (context, timeout) -> WebdriverProxyFactory
        .getFactory().proxyFor((WebDriver) context);
    ...
    private SearchContextType typeOf(SearchContext searchContext) {
        if (searchContext instanceof WebDriverFacade) return SearchContextType.WebDriverFacadeContext;
        if (searchContext instanceof WebDriver) return SearchContextType.WebDriverContext;
        if (searchContext instanceof WebElementFacade) return SearchContextType.WebElementFacadeContext;
        if (searchContext instanceof WebElement) return SearchContextType.WebElementFacadeContext;
    ...
    public WebElement findElementImmediately() {
        serenitycore.net.serenitybdd.core.annotations.locators.SmartAnnotations annotations = new serenitycore.net.serenitybdd.core.annotations.locators.SmartAnnotations(field, platform);
    ...
    private class SlowLoadingElementList extends SlowLoadableComponent<SlowLoadingElementList> {
        ...
        public String toString() {
            serenitycore.net.serenitybdd.core.annotations.locators.SmartAnnotations annotations = new SmartAnnotations(field, platform);
            By by = annotations.buildBy();
            return by.toString();
        }
    }
