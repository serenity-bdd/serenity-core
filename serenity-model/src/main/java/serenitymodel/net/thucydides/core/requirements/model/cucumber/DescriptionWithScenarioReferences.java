...
public class DescriptionWithScenarioReferences {
    ...
    public String forText(String line) {
        ...
        if (line.trim().startsWith(SCENARIO_WITH_TITLE_PREFIX)) {
            return ReferencedScenario.in(feature).withName(scenarioNameFrom(line,SCENARIO_WITH_TITLE_PREFIX)).asGivenWhenThen(
                ScenarioDisplayOption.WithTitle)
                    .orElse(highlighted(line));
        }
        ...
        if (line.trim().startsWith(EXAMPLES_WITH_TITLE_PREFIX)) {
            return ReferencedScenario.in(feature).withName(scenarioNameFrom(line,EXAMPLES_WITH_TITLE_PREFIX)).asExampleTable(
                ScenarioDisplayOption.WithTitle)
                    .orElse(ReferencedExampleTable.in(feature).withName(scenarioNameFrom(line,EXAMPLES_WITH_TITLE_PREFIX)).asExampleTable(
                        ScenarioDisplayOption.WithTitle)
                            .orElse(highlighted(line)))
                    ;
        }
